...
* `make fmt-ui` - Formats the classic UI source code
* `make validate-ui` - Runs tests and checks for the classic UI only
* `make fmt-ui-quick` - (experimental) Formats only changed classic UI source code
* `make validate-ui-quick` - (experimental) Runs tests and checks of changed classic UI code
* `make fmt-ui-hoard` - Formats the Hoard UI source code
* `make validate-ui-hoard` - Type-checks, lints and unit-tests the Hoard UI
* `make validate-ui-hoard-e2e` - Runs the Hoard UI Playwright tests against a production build. Downloads a browser on first run.
